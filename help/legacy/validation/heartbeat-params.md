...
Lijst met parameters voor Media Analytics die door Adobe worden verzameld en verwerkt op de Media Analytics-server (heartbeats):
...
| `s:asset:publisher` | MediaHeartboneConfig | (Vereist)<br/><br/>De uitgever van het element. |
...
| `s:user:mid` | Experience Cloud Org | (Vereist)<br/><br/>De waarde van de Experience cloud bezoeker-id van de gebruiker. |
...
| `s:aam:blob` | MediaHeartboneConfig | (Vereist)<br/><br/>Gegevens AAM verzonden op elke lading na aa_start |
...
| `l:stream:bitrate` | QoSInfo | (Vereist)<br/><br/>De huidige waarde van de streambitsnelheid (in bps). |
...
| `s:asset:resolver` | AdBreakInfo | (Vereist)<br/><br/>De advertentiesolver. |
...
| Naam | Gegevensbron | Beschrijving   |
| ---  | --- | --- |
| `s:stream:chapter_sid` | Media-SDK | (Vereist)<br/><br/>De unieke id die aan de afspeelinstantie van het hoofdstuk is gekoppeld.  <br/> **Opmerking:** Een hoofdstuk kan meerdere keren worden afgespeeld als gevolg van terugzoekbewerkingen die door de gebruiker worden uitgevoerd. |
| `s:stream:chapter_name` | ChapterInfo | (Optioneel)<br/><br/>De vriendelijke (leesbare) naam van het hoofdstuk. |
| `s:stream:chapter_id` | Media-SDK | (Vereist)<br/><br/>De unieke id van het hoofdstuk. Deze waarde wordt automatisch berekend op basis van de volgende formule: <br/>`MD5(video_id) +`<br/>` "_" +`<br/>`chapter_pos` |
| `l:stream:chapter_pos` | ChapterInfo | (Vereist)<br/><br/>De index van het hoofdstuk in de lijst met hoofdstukken (te beginnen met 1). |
| `l:stream:chapter_offset` | ChapterInfo | (Vereist)<br/><br/>De verschuiving van het hoofdstuk (uitgedrukt in seconden) binnen de hoofdinhoud, exclusief advertenties. |
