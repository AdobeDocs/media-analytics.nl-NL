...
| `Media.track` | **Syntaxis:** <br/><br/> `s.Media.track(mediaName)` <br/><br/>Verzend onmiddellijk de huidige videostaat, samen met om het even welk  `Media.trackVars` en Media.trackEvents u hebt bepaald. Deze methode wordt gebruikt binnen `Media.monitor`. <br/><br/>Zie Aanvullende metriek  [meten met Media.monitor.](https://experienceleague.adobe.com/docs/media-analytics/using/media-overview.html) <br/><br/>Roep  `Media.open` en roep  `Media.play` de video aan alvorens deze methode te roepen. Deze methode heeft de volgende parameter: <ul> <li> **mediaName**: De naam van de video. Dit moet overeenkomen met de naam die wordt gebruikt in `Media.open`.</li> </ul> Dit is de enige manier om extra variabelen te verzenden terwijl de video wordt afgespeeld. Het stelt het secondeninterval en de percenten milestone tellers aan nul terug om veelvoudige het volgen klappen te verhinderen. |
